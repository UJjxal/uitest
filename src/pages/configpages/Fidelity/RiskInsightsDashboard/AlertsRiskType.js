...
const AlertsRiskType=(props)=>{
    let colors=["#C1D3FE", "#CCDBFD", "#B6CCFE", "#D7E3FC", "#ABC4FF", "#E2EAFC"];
    ...
    const setDummyData=()=>{
        let data=[
            {value:12, name:'Reputational'},
            {value:15, name:'Legal Criminal'},
            {value:2, name:'GACP'},
            {value:20, name:'Legal Civil'},
            {value:16, name:'Regulatory'},
            {value:35, name:'Business Continuity'},
            {value:35, name:'Financial'},
        ];
        let total=0;
        data.forEach(v=>{
            total+=v.value;
    ...
    useEffect(()=>{
        ...
        .then((result)=>{
            if(result.data.code===200){
                let data=[
                    {value:0, name:'Reputational'},
                    {value:0, name:'Legal Criminal'},
                    {value:0, name:'GACP'},
                    {value:0, name:'Legal Civil'},
                    {value:0, name:'Regulatory'},
                    {value:0, name:'Business Continuity'},
                ...
                result.data.response.data.forEach(v=>{
                    v.count_data.forEach(c=>{
                        ...
                        if(c.risk_type==='Business Continuity'){
                            data[5].value+=c.criteria_count;
                        }
                        if(c.risk_type==='Financial'){
                            data[6].value+=c.criteria_count;
                        }
                    })
                });
    ...
    return (
        <div className="whitebx">
            <div className="heading uc text-center fs18">Action Taken by Risk Type</div>
