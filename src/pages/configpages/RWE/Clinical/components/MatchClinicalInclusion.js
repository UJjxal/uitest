...
const MatchClinicalInclusion = (props) => {
		const setCancel = () => {
        props.toggleExclusion();        
	};	
    ...
    const [indicationList, setindicationList] = useState(null);
    const [demographicList, setdemographicList] = useState(null);
    const [vitalList, setvitalList] = useState(null);	
    ...
    useEffect(() => {
		const fetchindicationData = async () => {
			const result = await axios(`${CONTEXT}/Clinical/RWE/IndicationCriteriaNCT00645593.json`);
			setindicationList(result.data);
        };
        const fetchdemoData = async () => {
			const result = await axios(`${CONTEXT}/Clinical/RWE/DemographicCriteriaNCT00645593.json`);
			setdemographicList(result.data);
        };
        const fetchvitalData = async () => {
			const result = await axios(`${CONTEXT}/Clinical/RWE/VitalCriteriaNCT00645593.json`);
...
<MDBCol>
<MDBCard className="" style={{overflowY: 'scroll', height: '23rem', "margin-left": "-12px" }}>
...
export default MatchClinicalInclusion;
