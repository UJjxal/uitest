...
const EHRSCA1Inclusion = (props) => {
		const setCancel = () => {
        props.toggleEHRSCA1();        
    };	
    const [EHRSCA1data, setEHRSCAdata] = useState(null);

    
    useEffect(() => {
		const fetchEHRSCA1Data = async () => {
		...
		return (
		<MDBModal isOpen={props.EHRSCAdisplay1} toggle={() => setCancel()} className="cascading-modal mt-5" size="fluid">
			<div className="modal-header primary-color white-text mb-0">
				<h4 className="title">
					<MDBIcon icon="dice-d6 fa-2x" /> EHR SCA1 
				</h4>
...
{EHRSCA1data ? (
EHRSCA1data.rows.map((row, index) => {
return (
<tr>
<td
...
export default EHRSCA1Inclusion;
