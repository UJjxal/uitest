...
const EHRSCA2Inclusion = (props) => {
		const setCancel = () => {
        props.toggleEHRSCA2();        
    };	
    const [EHRSCA2data, setEHRSCAdata] = useState(null);

    
    useEffect(() => {
		const fetchEHRSCA2Data = async () => {
			const result = await axios(`${CONTEXT}/Clinical/RWE/viewEHRSCA2.json`);
			setEHRSCAdata(result.data);
        };
        fetchEHRSCA2Data();        
	}, []);
		return (
		<MDBModal isOpen={props.EHRSCAdisplay2} toggle={() => setCancel()} className="cascading-modal mt-5" size="fluid">
			<div className="modal-header primary-color white-text mb-0">
				<h4 className="title">
					<MDBIcon icon="dice-d6 fa-2x" /> EHR SCA2 
				</h4>
...
{EHRSCA2data ? (
EHRSCA2data.rows.map((row, index) => {
return (
<tr>
<td
...
export default EHRSCA2Inclusion;
